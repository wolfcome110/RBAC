...
namespace EF.Common
{
    public class XMLHelper
    {
        ...
        /// <param name="xmlNodeName">要匹配xmlNodeName的节点名称</param>
        /// <param name="innerText">节点文本值</param>
        /// <param name="ht">要匹配xmlAttributeName的属性名称键值对</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateXmlNodeByXPath(string xmlFileName, string xpath, string xmlNodeName, string innerText, Hashtable ht)
        {
            bool isSuccess = false;
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    //新增xmlAttributeName的属性名称键值对
                    foreach (DictionaryEntry item in ht)
                    {
                        XmlAttribute xmlAttribute = xmlDoc.CreateAttribute(item.Key.ToString());
                        xmlAttribute.Value = Convert.ToString(item.Value);
                        subElement.Attributes.Append(xmlAttribute);
                    }
        ...
        /// <param name="ht">要匹配xmlAttributeName的属性名称键值对</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateOrUpdateXmlAttributeByXPath(string xmlFileName, string xpath, Hashtable ht)
        {
            bool isSuccess = false;
            XmlDocument xmlDoc = new XmlDocument();
            try
            {
                ...
                if (xmlNode != null)
                {
                    ...
                    foreach (XmlAttribute attribute in xmlNode.Attributes)
                    {
                        if (ht.ContainsKey(attribute.Name))
                        {
                            //节点中存在此属性则更新
                            attribute.Value = ht[attribute.Name].ToString();
                            ht.Remove(attribute.Name);
                        }
                    }
                    ...
                    foreach (DictionaryEntry item in ht)
                    {
                        //节点中不存在此属性则创建
                        XmlAttribute xmlAttribute = xmlDoc.CreateAttribute(item.Key.ToString());
                        xmlAttribute.Value = item.Value.ToString();
                        xmlNode.Attributes.Append(xmlAttribute);
                    }
                }
                xmlDoc.Save(xmlFileName); //保存到XML文档
